...
class DenseNetEfficient(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(4, 4, 4, 4), compression=0.5,
        ...
        # First convolution
        self.features = nn.Sequential(OrderedDict([('conv0', nn.Conv2d(4, num_init_features, kernel_size=3, stride=1,
        ...
        final_layer = nn.Sequential(OrderedDict([
            ('final_denseblock', _DenseBlock(num_layers=2, num_input_features=num_features, bn_size=bn_size, growth_rate=4,
                                             drop_rate=drop_rate)),
            ('final_conv', nn.Conv2d(in_channels=num_features+2*4, out_channels=4, kernel_size=(1, 1), padding=0,))
            # ('tanh', nn.Tanh())
        ]))
